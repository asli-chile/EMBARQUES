...
/** Superadmin o admin. */
export async function requireAdminOrAbove(cookies: AstroCookies): Promise<
  | { authorized: false; status: 401 | 403 | 503; error: string }
  | { authorized: true; admin: ReturnType<typeof createAdminClient>; userId: string }
> {
  const supabase = createClient(cookies);
  ...
  if (rol !== "superadmin" && rol !== "admin") {
    return { authorized: false, status: 403, error: "Solo administradores pueden gestionar esta configuración" };
  }
  try {
    const admin = createAdminClient();
    return { authorized: true, admin, userId: user.id };
